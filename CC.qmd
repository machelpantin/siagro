...
def build_regional_chart(df, country_name):
    ...
    if len(available_sub_regions) > 0:
        ...
        all_regions_data = df[df['Subnational administrative division'].isin(available_sub_regions)].copy()
        if not all_regions_data.empty:
            calculated_national = all_regions_data.groupby('Months')['value'].mean().reset_index()
            calculated_national['month_num'] = calculated_national['Months'].map(month_order)
    ...
    if not national_data.empty and calculated_national_temps is not None:
        ...
        if diff > 3.0:
            print(f"  ⚠️  {country_name}: 国家数据与子区域差异较大 ({diff:.2f}°C)，使用子区域计算的平均值")
            use_calculated = True
