...
## Temperatura Promedio por País (1960-1990)
...
>! Nota: El gráfico a continuación no funciona por el momento.
...
# Identify the correct column names by looking for relevant columns
region_col = None
month_col = None
...
#print(f"DEBUG: Identified region column: {region_col}")
#print(f"DEBUG: Identified month column: {month_col}")
...
def build_regional_chart(df, country_name):
    ...
    # Use the globally identified column names
    if region_col is None or month_col is None:
        print(f"ERROR: Missing required columns. region_col={region_col}, month_col={month_col}")
        return None
    
    if region_col not in df.columns or month_col not in df.columns:
        return None
    
    # Get national level data (exact match)
    national_data = df[df[region_col] == country_name].copy()
    ...
    all_regions_list = df[region_col].unique()
    ...
    # Calculate national average from sub-regions if available
    # This is more accurate than using potentially inconsistent national-level data
    calculated_national_temps = None
    if len(available_sub_regions) > 0:
        # Collect all sub-region data by month
        all_regions_data = df[df[region_col].isin(available_sub_regions)].copy()
        if not all_regions_data.empty:
            calculated_national = all_regions_data.groupby(month_col)['value'].mean().reset_index()
            calculated_national['month_num'] = calculated_national[month_col].map(month_order)
            calculated_national = calculated_national.sort_values('month_num')
            calculated_national_temps = calculated_national['value'].round(2).tolist()
    
    # Add national average line (black, always visible, cannot be hidden)
    # Prefer calculated average from sub-regions if available and consistent
    use_calculated = False
    if not national_data.empty and calculated_national_temps is not None:
        # Check if national data is consistent with sub-regions
        national_avg = national_data['value'].mean()
        calculated_avg = pd.Series(calculated_national_temps).mean()
        diff = abs(national_avg - calculated_avg)
        
        # If difference is too large (>3°C), use calculated average
        if diff > 3.0:
            print(f"  ⚠️  {country_name}: National data and sub-regional variations differ considerably. ({diff:.2f}°C). Using the average calculated using sub-regions")
            use_calculated = True
            national_temps = calculated_national_temps
        else:
            # Use official national data
            national_monthly = national_data.groupby(month_col)['value'].mean().reset_index()
            national_monthly['month_num'] = national_monthly[month_col].map(month_order)
            national_monthly = national_monthly.sort_values('month_num')
            national_temps = national_monthly['value'].round(2).tolist()
    elif calculated_national_temps is not None:
        # No national data, use calculated
        use_calculated = True
        national_temps = calculated_national_temps
    elif not national_data.empty:
    ...
    if national_temps is not None and len(national_temps) > 0:
        all_temps_list.extend(national_temps)
        # Keep as simple 1D list - PyEcharts will match with xaxis categories
        national_temps = [float(x) for x in national_temps]
        
        label_suffix = " (Calculated from Sub-regions)" if use_calculated else " (National Avg)"
        chart.add_yaxis(
            f"{country_name}{label_suffix}",
            national_temps,
            is_smooth=True,
            symbol="circle",
            symbol_size=8,
            ...
            label_opts=opts.LabelOpts(
                is_show=True,
                position="top",
                formatter=JsCode("function(params) { return params.value.toFixed(1) + '°C'; }")
            )
        )
    ...
    for idx, region in enumerate(available_sub_regions):
        ...
        monthly_avg = region_data.groupby(month_col)['value'].mean().reset_index()
        monthly_avg['month_num'] = monthly_avg[month_col].map(month_order)
        monthly_avg = monthly_avg.sort_values('month_num')
        
        temps = monthly_avg['value'].round(2).tolist()
        all_temps_list.extend(temps)
        # Keep as simple 1D list
    ...
    chart.set_global_opts(
        title_opts=opts.TitleOpts(
            title=f"Regional vs Nacional: {country_name}",
            subtitle="Línea negra = Media nacional | Líneas de color = Subregiones | Fuente: CEPALSTAT",
...
# Save to file instead of rendering in notebook
tab.render("charts/temperature_by_region.html")
```

```{=html}
 <iframe src="charts/temperature_by_region.html" width="100%" height="600px" frameborder="0"></iframe>
```

## Comparación de Temperatura entre Países
...
def build_multi_country_line(df):
    ...
    for idx, country in enumerate(country_names):
        country_data = df_filtered[df_filtered[region_col] == country].copy()
        
        if country_data.empty or month_col not in country_data.columns:
            continue
        
        # Calculate average by month
        monthly_avg = country_data.groupby(month_col)['value'].mean().reset_index()
        monthly_avg['month_num'] = monthly_avg[month_col].map(month_order)
        monthly_avg = monthly_avg.sort_values('month_num')
    ...
    chart.set_global_opts(
        title_opts=opts.TitleOpts(
            title="Comparación de temperaturas mensuales",
            subtitle="Haz clic en la leyenda para mostrar u ocultar los países | Fuente: CEPALSTAT"
        ),
...
line_chart.render("charts/temperature_comparison.html")
```

```{=html}
<iframe src="charts/temperature_comparison.html" width="100%" height="600px" frameborder="0"></iframe>
```

## Ranking de Países por Temperatura Promedio
...
def build_ranking_chart(df):
    ...
    for country in country_names:
        country_data = df[df[region_col] == country]
    ...
    chart.set_global_opts(
        title_opts=opts.TitleOpts(
            title="Temperatura media por país (1960-1990)",
            subtitle="América Central y México | Fuente: CEPALSTAT"
        ),
...
if ranking_chart:
    ranking_chart.render("charts/temperature_ranking.html")
```

```{=html}
<iframe src="charts/temperature_ranking.html" width="100%" height="500px" frameborder="0"></iframe>
```
